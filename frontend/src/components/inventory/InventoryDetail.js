                      ...
                      <tbody className="divide-y divide-gray-700">
                        {transactions.map((transaction) => (
                          <tr key={transaction.id}>
                            ...
                            <td className={`px-4 py-3 whitespace-nowrap text-sm font-medium ${getTransactionTypeColorClass(transaction.transaction_type)}`}>
                              {getTransactionTypeText(transaction.transaction_type)}
                            </td>
                            <td className={`px-4 py-3 whitespace-nowrap text-sm text-right font-medium ${getTransactionTypeColorClass(transaction.transaction_type)}`}>
                              {transaction.transaction_type === 'out' ? '-' : ''}
                              {parseFloat(transaction.quantity).toFixed(2)} {item.unit}
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm">
