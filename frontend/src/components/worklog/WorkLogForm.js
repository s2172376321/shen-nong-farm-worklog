...
const WorkLogForm = ({ onSubmitSuccess }) => {
  ...
  const loadLocations = useCallback(async () => {
    try {
      ...
      if (Array.isArray(locationData)) {
        ...
        console.log(`成功載入 ${uniqueAreas.length} 個區域和 ${locationData.length} 個位置`);
      } else {
  ...
  // 加載分區位置數據 - 新增函數
  const loadLocationsByArea = useCallback(async () => {
    try {
      console.log('嘗試加載分區位置數據');
      const areaData = await fetchLocationsByArea();
      if (Array.isArray(areaData)) {
        setAreas(areaData.map(area => area.areaName));
        console.log(`成功載入 ${areaData.length} 個區域`);
        
        // 提取所有位置數據
        let allPositionsData = [];
        areaData.forEach(area => {
          if (Array.isArray(area.locations)) {
            area.locations.forEach(location => {
              allPositionsData.push({
                區域名稱: area.areaName,
                位置代號: location.locationCode,
                位置名稱: location.locationName
              });
            });
          }
        });
        ...
        console.log(`成功提取 ${allPositionsData.length} 個位置`);
      } else {
        console.error('分區位置資料格式不正確', areaData);
        setAreas([]);
        setAllPositions([]);
      }
    } catch (err) {
      console.error('載入分區位置資料失敗', err);
      setAreas([]);
      setAllPositions([]);
    }
  }, []);
  ...
  const calculateWorkHours = useCallback((logs) => {
    ...
    logs.forEach((log, index) => {
      // 確保時間格式一致
      let startTime = log.start_time || log.startTime || '';
      let endTime = log.end_time || log.endTime || '';
      
      // 標準化時間格式 - 確保只是 HH:MM 格式
      ...
      totalMinutes += Math.max(0, logMinutes); // 確保不會加入負數分鐘
    });
  ...
  useEffect(() => {
    if (!dataLoaded) {
      const loadBaseData = async () => {
        try {
          console.log('開始載入基礎數據...');
          setIsLoading(true);
          setFormErrors({});
          
          await Promise.all([
            loadLocationsByArea(), // 修改為使用分區位置資料
            loadWorkCategories(),
            loadProducts()
          ]);
  ...
  }, [loadLocationsByArea, loadWorkCategories, loadProducts, dataLoaded]);
  ...
  useEffect(() => {
    if (selectedArea) {
      ...
      console.log(`區域 "${selectedArea}" 有 ${filteredPositions.length} 個位置`);
    } else {
      setPositions([]);
    }
  ...
  const handleWorkCategoryChange = (e) => {
    ...
    console.log('選擇工作類別:', categoryCode);  
  ...
  const handleProductCategoryChange = (e) => {
    ...
    console.log('選擇產品類別:', category);
    
    // 依據產品類別篩選產品
    try {
      const filtered = products.filter(p => p.中分類 && p.中分類.toString() === category);
      console.log(`篩選出 ${filtered.length} 個產品`);
