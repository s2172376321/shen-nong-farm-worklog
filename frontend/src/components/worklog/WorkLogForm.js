...
const WorkLogForm = ({ onSubmitSuccess }) => {
  ...
  const handleProductSelect = (product) => {
    ...
    setWorkLog(prev => {
      ...
      // 如果是種植工作，則自動設置作物名稱為種子/種苗的名稱
      if (prev.work_category_name === '種植' && product.規格) {
        // 從規格中提取作物名稱 (例如："A菜-250g" -> "A菜")
        let cropName = product.規格;
        
        // 嘗試處理常見的格式，移除重量或包裝信息
        if (cropName.includes('-')) {
          cropName = cropName.split('-')[0].trim();
        } else if (cropName.includes(' ')) {
          cropName = cropName.split(' ')[0].trim();
        }
        
        // 設置作物名稱
        newState.crop = cropName;
      }
