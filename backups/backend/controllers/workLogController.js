...
async createWorkLog(req, res) {
  try {
    ...
    if (!actualStartTime || !actualEndTime || 
        !timeRegex.test(actualStartTime) || !timeRegex.test(actualEndTime)) {
    ...
    const formattedStartTime = actualStartTime + ':00';
    const formattedEndTime = actualEndTime + ':00';
    ...
    // 計算工作時數
    const workHoursResult = calculateWorkHours(actualStartTime, actualEndTime);
